    ...
    for _, row in df.iterrows():
        ...
        for target in connections:
            try:
                ...
                print(
                    f"Advertencia: Nodo inválido '{target}' en fila {node}, ignorando esta conexión."
                )
    return G
...
def find_key_nodes(G, top_n=250):
    ...
    if G is not None and isinstance(G, nx.Graph):
        key_nodes = sorted(
            G.nodes, key=lambda node: len(list(G.neighbors(node))), reverse=True
        )[:top_n]
        if key_nodes:
...
def draw_subgraph(G, local_subgraph_nodes, key_node, canvas_tag):
    ...
    subgraph_positions = {
        node: (x * 250, y * 250) for node, (x, y) in subgraph_layout.items()
    }  #
    ...
    dpg.draw_text(
        (key_node_x + offset_x + 12, key_node_y + offset_y - 12),
        str(key_node),
        color=(255, 255, 255),
        parent=canvas_tag,
        size=14,
    ...
    for neighbor in local_subgraph_nodes:
        ...
        if neighbor in subgraph_positions:
            ...
            if subgraph.has_edge(
                key_node, neighbor
            ):  # Usamos .has_edge() para verificar si hay una conexión
                key_node_x, key_node_y = subgraph_positions[key_node]
...
def draw_full_graph(G, canvas_tag, node_info):
    ...
    layout = nx.spring_layout(
        G, k=0.2, scale=1.5
    )  # Ajustar k para aumentar separación de los nodos
    ...
    for node in G.nodes:
        ...
        dpg.draw_circle(
            (x + offset_x, y + offset_y), 10, color=border_color, parent=canvas_tag
        )
        dpg.draw_circle(
            (x + offset_x, y + offset_y), 8, color=color, parent=canvas_tag, fill=color
        )

    # Actualizar el mensaje con la cantidad de nodos clave identificados
    dpg.set_value(
        node_info,
        f"La cantidad de nodos clave identificados en el grafo es: {len(key_nodes)}",
    )
...
def next_node(G, canvas_tag, node_info):
    ...
    if key_nodes:
        ...
        dpg.set_value(
            node_info,
            f"El nodo clave actual es {key_node} y tiene {num_connections} conexiones.",
        )
    ...
    if G is not None:
        ...
        if key_nodes:
            ...
            with dpg.window(label="Información del Nodo", width=300, height=100):
                node_info = dpg.add_text(
                    f"El nodo clave actual es {key_node} y tiene {len(list(G.neighbors(key_node)))} conexiones."
                )
        ...
        with dpg.window(label="Simulación de Nodos Claves", width=1400, height=900):
            with dpg.group(horizontal=True):
                dpg.add_button(
                    ...
                    callback=lambda: next_node(G, "canvas", node_info),
                )
                dpg.add_button(
                    label="Identificar Todos los Nodos Clave",
                    callback=lambda: draw_full_graph(G, "canvas", node_info),
                )
            with dpg.group(horizontal=True):
                node_info = dpg.add_text(
                    f"El nodo clave actual es {key_node} y tiene {len(list(G.neighbors(key_node)))} conexiones."
